...
public class AiKnowledgeApplicationServiceImpl implements AiKnowledgeApplicationService {
    ...
    @Override public AiKnowledgeBaseResponse knowledgeBase(Long id) {
    ...
    public Long createKnowledgeBase(AiKnowledgeBaseRequest request) {
        ...
        base.setStatus("DRAFT"); base.setDocumentCount(0); base.setFaqCount(0); base.setChunkCount(0);
    ...
    @Override public void setKnowledgeBaseEnabled(Long id, boolean enabled) {
        AiKnowledgeBase base = requireBase(id);
        base.setEnabled(enabled); base.setStatus(enabled ? aggregateStatus(id) : "DISABLED");
    ...
    public void rebuildKnowledgeBase(Long id, Long embeddingModelId) {
        ...
        if (model.getVectorDimension() == null || model.getVectorDimension() <= 0) throw new ServiceException("目标向量模型尚未测试，无法确定维度");
        ...
        if (rebuildTasks.isEmpty()) {
            base.setEmbeddingModelId(model.getId()); base.setCollectionName(collection); base.setStatus("DRAFT"); baseMapper.updateById(base); return;
        }
        base.setPendingEmbeddingModelId(model.getId()); base.setPendingCollectionName(collection); base.setRebuildBatchId(batchId);
        base.setStatus("INDEXING"); base.setFailureReason(null); baseMapper.updateById(base);
    ...
    public Long uploadDocument(Long knowledgeBaseId, Long documentId, MultipartFile file) {
        ...
        byte[] bytes;
        try { bytes = file.getBytes(); } catch (Exception e) { throw new ServiceException("读取知识文件失败：" + e.getMessage()); }
        ...
        if (documentId == null) {
            ...
            document.setStatus("UPLOADED"); document.setEnabled(true);
            documentMapper.insert(document);
        } else {
            document = requireDocument(documentId);
            if (!Objects.equals(document.getKnowledgeBaseId(), knowledgeBaseId)) throw new ServiceException("文档不属于当前知识库");
            document.setStatus("PROCESSING"); documentMapper.updateById(document);
        }
        ...
        version.setKnowledgeBaseId(knowledgeBaseId); version.setDocumentId(document.getId()); version.setVersionNo(next);
        version.setOssId(oss.getOssId()); version.setOriginalFileName(file.getOriginalFilename()); version.setContentType(file.getContentType());
        version.setFileSize(file.getSize()); version.setChecksum(hexSha256(bytes)); version.setParseStatus("PENDING"); version.setIndexStatus("PENDING");
    ...
    @Override public void deleteDocument(Long id) {
        AiKnowledgeDocument document = requireDocument(id);
        document.setEnabled(false); document.setStatus("DISABLED"); documentMapper.updateById(document);
        createAndDispatch("DELETE_INDEX", document.getKnowledgeBaseId(), document.getId(), document.getCurrentVersionId(), null, null);
    }

    @Override public List<AiKnowledgeChunk> chunks(Long documentId) {
        requireDocument(documentId);
    ...
    public Long createFaq(Long knowledgeBaseId, AiKnowledgeFaqRequest request) {
        ...
        faq.setKnowledgeBaseId(knowledgeBaseId); faq.setFaqCode(normalizeCode(request.getFaqCode())); faq.setFaqName(request.getFaqName().trim());
        faq.setAnswerMode(answerMode(request.getAnswerMode())); faq.setStatus("DRAFT"); faq.setEnabled(request.getEnabled() == null || request.getEnabled());
    ...
    public void updateFaq(Long id, AiKnowledgeFaqRequest request) {
        ...
        faq.setFaqCode(normalizeCode(request.getFaqCode())); faq.setFaqName(request.getFaqName().trim());
        faq.setAnswerMode(answerMode(request.getAnswerMode())); faq.setEnabled(request.getEnabled() == null || request.getEnabled());
        faq.setStatus("INDEXING"); faq.setVersion(request.getVersion());
        if (faqMapper.updateById(faq) != 1) throw new ServiceException("FAQ 已被其他用户修改，请刷新后重试");
        createFaqVersion(faq, request);
    }

    @Override public void deleteFaq(Long id) {
        AiKnowledgeFaq faq = requireFaq(id);
        faq.setEnabled(false); faq.setStatus("DISABLED"); faqMapper.updateById(faq);
        createAndDispatch("DELETE_FAQ_INDEX", faq.getKnowledgeBaseId(), null, null, faq.getId(), faq.getCurrentVersionId());
    }

    @Override public void setFaqEnabled(Long id, boolean enabled) {
        AiKnowledgeFaq faq = requireFaq(id);
        faq.setEnabled(enabled); faq.setStatus(enabled ? (faq.getCurrentVersionId() == null ? "DRAFT" : "READY") : "DISABLED");
        faqMapper.updateById(faq);
    }

    @Override public List<AiKnowledgeTaskResponse> tasks(Long knowledgeBaseId) {
    ...
    @Override public void retryTask(Long taskId) {
        ...
        task.setStatus("PENDING"); task.setNextRetryAt(null); task.setFailureReason(null); task.setLeaseOwner(null); task.setLeaseExpiresAt(null);
        taskMapper.updateById(task); dispatcher.dispatch(task.getId(), TenantHelper.getTenantId());
    }

    @Override public List<AiKnowledgeSearchHitResponse> search(Long knowledgeBaseId, AiKnowledgeSearchRequest request) {
        AiKnowledgeBase base = requireEnabledBase(knowledgeBaseId);
    ...
    private void createFaqVersion(AiKnowledgeFaq faq, AiKnowledgeFaqRequest request) {
        ...
        if (new HashSet<>(normalized).size() != normalized.size()) throw new ServiceException("标准问题和相似问法存在重复");
        ...
        version.setKnowledgeBaseId(faq.getKnowledgeBaseId()); version.setFaqId(faq.getId()); version.setVersionNo(next);
        version.setStandardQuestion(request.getStandardQuestion().trim()); version.setNormalizedQuestion(normalized.get(0));
        version.setStandardAnswer(request.getStandardAnswer().trim()); version.setQuestionHash(KnowledgeTextUtils.sha256(normalized.get(0)));
        version.setAnswerHash(KnowledgeTextUtils.sha256(version.getStandardAnswer())); version.setIndexStatus("PENDING");
        faqVersionMapper.insert(version);
        for (int i = 1; i < normalized.size(); i++) {
            AiKnowledgeFaqAlias alias = new AiKnowledgeFaqAlias();
            alias.setKnowledgeBaseId(faq.getKnowledgeBaseId()); alias.setFaqId(faq.getId()); alias.setFaqVersionId(version.getId());
            alias.setAliasQuestion(request.getAliases().get(i - 1).trim()); alias.setNormalizedQuestion(normalized.get(i));
            alias.setQuestionHash(KnowledgeTextUtils.sha256(normalized.get(i))); alias.setQdrantPointId(pointId("faq-alias", alias.getQuestionHash()));
            alias.setIndexState("STAGING"); faqAliasMapper.insert(alias);
        }
        faq.setStatus("INDEXING"); faqMapper.updateById(faq);
    ...
    private void ensureQuestionsAvailable(Long kbId, Long faqId, List<String> questions) {
        ...
        Set<Long> versions = new HashSet<>(); activeFaqs.forEach(item -> versions.add(item.getCurrentVersionId()));
        ...
        if (questions.stream().anyMatch(used::contains)) throw new ServiceException("FAQ 问题或相似问法已在当前知识库中使用");
    }
    ...
    private AiKnowledgeTask createTask(String type, Long kbId, Long documentId, Long documentVersionId, Long faqId, Long faqVersionId) {
        ...
        task.setTaskType(type); task.setKnowledgeBaseId(kbId); task.setDocumentId(documentId); task.setDocumentVersionId(documentVersionId);
        task.setFaqId(faqId); task.setFaqVersionId(faqVersionId); task.setStatus("PENDING"); task.setRetryCount(0);
        task.setProgressTotal(0); task.setProgressCompleted(0); taskMapper.insert(task); return task;
    }
    private AiKnowledgeTask createRebuildTask(String type, AiKnowledgeBase base, Long documentId, Long documentVersionId,
        ...
        task.setTargetEmbeddingModelId(modelId); task.setTargetCollectionName(collection); task.setRebuildBatchId(batchId);
    ...
    private void fillBase(AiKnowledgeBase base, AiKnowledgeBaseRequest request) {
        ...
        base.setKnowledgeCode(normalizeCode(request.getKnowledgeCode())); base.setKnowledgeName(request.getKnowledgeName().trim());
        base.setDescription(request.getDescription()); base.setEmbeddingModelId(request.getEmbeddingModelId());
        base.setChunkSize(size); base.setChunkOverlap(overlap); base.setDefaultTopK(request.getDefaultTopK() == null ? 5 : request.getDefaultTopK());
    ...
    private void validateFile(MultipartFile file) {
        ...
        if (file.getSize() > max) throw new ServiceException("知识文件不能超过 " + properties.getMaxDocumentSizeMb() + "MB");
        if (!SUPPORTED_SUFFIXES.contains(suffix(file.getOriginalFilename()))) throw new ServiceException("仅支持 TXT、MD、PDF、DOCX、XLSX");
    }
    ...
    private AiKnowledgeBase requireBase(Long id) { AiKnowledgeBase item = baseMapper.selectById(id); if (item == null) throw new ServiceException("知识库不存在"); return item; }
    private AiKnowledgeBase requireEnabledBase(Long id) { AiKnowledgeBase item = requireBase(id); if (!Boolean.TRUE.equals(item.getEnabled())) throw new ServiceException("知识库已停用"); return item; }
    private AiKnowledgeDocument requireDocument(Long id) { AiKnowledgeDocument item = documentMapper.selectById(id); if (item == null) throw new ServiceException("知识文档不存在"); return item; }
    private AiKnowledgeFaq requireFaq(Long id) { AiKnowledgeFaq item = faqMapper.selectById(id); if (item == null) throw new ServiceException("FAQ 不存在"); return item; }
    private AiModel requireEmbeddingModel(Long id) { AiModel item = modelMapper.selectById(id); if (item == null || !"EMBEDDING".equals(item.getCapability()) || !Boolean.TRUE.equals(item.getEnabled())) throw new ServiceException("向量模型不存在或未启用"); return item; }
    private AiModelProvider requireEnabledProvider(Long id) { AiModelProvider item = providerMapper.selectById(id); if (item == null || !Boolean.TRUE.equals(item.getEnabled())) throw new ServiceException("模型服务商不存在或未启用"); return item; }
    private void ensureBaseCode(String code, Long exclude) { if (baseMapper.selectCount(new LambdaQueryWrapper<AiKnowledgeBase>().eq(AiKnowledgeBase::getKnowledgeCode, normalizeCode(code)).ne(exclude != null, AiKnowledgeBase::getId, exclude)) > 0) throw new ServiceException("知识库编码已存在"); }
    private void ensureFaqCode(Long kb, String code, Long exclude) { if (faqMapper.selectCount(new LambdaQueryWrapper<AiKnowledgeFaq>().eq(AiKnowledgeFaq::getKnowledgeBaseId, kb).eq(AiKnowledgeFaq::getFaqCode, normalizeCode(code)).ne(exclude != null, AiKnowledgeFaq::getId, exclude)) > 0) throw new ServiceException("FAQ 编码已存在"); }
    private String normalizeCode(String value) { return value.trim().toUpperCase(Locale.ROOT); }
    private String answerMode(String value) { String result = StringUtils.isBlank(value) ? "DIRECT" : value.trim().toUpperCase(Locale.ROOT); if (!Set.of("DIRECT", "CONTEXT").contains(result)) throw new ServiceException("FAQ 回答模式不支持"); return result; }
    private String suffix(String fileName) { if (fileName == null || !fileName.contains(".")) return ""; return fileName.substring(fileName.lastIndexOf('.') + 1).toLowerCase(Locale.ROOT); }
    private String hexSha256(byte[] bytes) { try { return HexFormat.of().formatHex(MessageDigest.getInstance("SHA-256").digest(bytes)); } catch (Exception e) { throw new IllegalStateException(e); } }
    private String pointId(String type, String value) { return UUID.nameUUIDFromBytes((type + ":" + value).getBytes(java.nio.charset.StandardCharsets.UTF_8)).toString(); }
    private String collectionName(String tenantId, AiModel model) { String hash = KnowledgeTextUtils.sha256(tenantId).substring(0, 12); return "cnx_kb_" + hash + "_" + model.getId() + "_" + model.getVectorDimension(); }
    private AiKnowledgeBaseResponse baseResponse(AiKnowledgeBase item, String modelName) { AiKnowledgeBaseResponse r = new AiKnowledgeBaseResponse(); r.setId(item.getId()); r.setKnowledgeCode(item.getKnowledgeCode()); r.setKnowledgeName(item.getKnowledgeName()); r.setDescription(item.getDescription()); r.setEmbeddingModelId(item.getEmbeddingModelId()); r.setEmbeddingModelName(modelName); r.setCollectionName(item.getCollectionName()); r.setChunkSize(item.getChunkSize()); r.setChunkOverlap(item.getChunkOverlap()); r.setDefaultTopK(item.getDefaultTopK()); r.setScoreThreshold(item.getScoreThreshold()); r.setStatus(item.getStatus()); r.setDocumentCount(item.getDocumentCount()); r.setFaqCount(item.getFaqCount()); r.setChunkCount(item.getChunkCount()); r.setLastIndexedAt(item.getLastIndexedAt()); r.setFailureReason(item.getFailureReason()); r.setEnabled(item.getEnabled()); r.setVersion(item.getVersion()); return r; }
    private AiKnowledgeDocumentResponse documentResponse(AiKnowledgeDocument item) { AiKnowledgeDocumentResponse r = new AiKnowledgeDocumentResponse(); r.setId(item.getId()); r.setKnowledgeBaseId(item.getKnowledgeBaseId()); r.setDocumentName(item.getDocumentName()); r.setDocumentType(item.getDocumentType()); r.setCurrentVersionId(item.getCurrentVersionId()); r.setStatus(item.getStatus()); r.setEnabled(item.getEnabled()); r.setCreateTime(item.getCreateTime()); AiKnowledgeDocumentVersion v = item.getCurrentVersionId() == null ? documentVersionMapper.selectList(new LambdaQueryWrapper<AiKnowledgeDocumentVersion>().eq(AiKnowledgeDocumentVersion::getDocumentId,item.getId()).orderByDesc(AiKnowledgeDocumentVersion::getVersionNo).last("limit 1")).stream().findFirst().orElse(null) : documentVersionMapper.selectById(item.getCurrentVersionId()); if(v!=null){r.setVersionNo(v.getVersionNo());r.setParseStatus(v.getParseStatus());r.setIndexStatus(v.getIndexStatus());r.setChunkCount(v.getChunkCount());r.setFailureReason(v.getFailureReason());} return r; }
    private AiKnowledgeFaqResponse faqResponse(AiKnowledgeFaq item) { AiKnowledgeFaqResponse r = new AiKnowledgeFaqResponse(); r.setId(item.getId()); r.setKnowledgeBaseId(item.getKnowledgeBaseId()); r.setFaqCode(item.getFaqCode()); r.setFaqName(item.getFaqName()); r.setCurrentVersionId(item.getCurrentVersionId()); r.setStatus(item.getStatus()); r.setAnswerMode(item.getAnswerMode()); r.setEnabled(item.getEnabled()); r.setVersion(item.getVersion()); r.setCreateTime(item.getCreateTime()); AiKnowledgeFaqVersion v = item.getCurrentVersionId()==null?faqVersionMapper.selectList(new LambdaQueryWrapper<AiKnowledgeFaqVersion>().eq(AiKnowledgeFaqVersion::getFaqId,item.getId()).orderByDesc(AiKnowledgeFaqVersion::getVersionNo).last("limit 1")).stream().findFirst().orElse(null):faqVersionMapper.selectById(item.getCurrentVersionId()); if(v!=null){r.setVersionNo(v.getVersionNo());r.setStandardQuestion(v.getStandardQuestion());r.setStandardAnswer(v.getStandardAnswer());r.setIndexStatus(v.getIndexStatus());r.setFailureReason(v.getFailureReason());r.setAliases(faqAliasMapper.selectList(new LambdaQueryWrapper<AiKnowledgeFaqAlias>().eq(AiKnowledgeFaqAlias::getFaqVersionId,v.getId())).stream().map(AiKnowledgeFaqAlias::getAliasQuestion).toList());} return r; }
    private AiKnowledgeTaskResponse taskResponse(AiKnowledgeTask item) { AiKnowledgeTaskResponse r=new AiKnowledgeTaskResponse(); r.setId(item.getId());r.setTaskType(item.getTaskType());r.setKnowledgeBaseId(item.getKnowledgeBaseId());r.setDocumentId(item.getDocumentId());r.setFaqId(item.getFaqId());r.setStatus(item.getStatus());r.setRetryCount(item.getRetryCount());r.setProgressTotal(item.getProgressTotal());r.setProgressCompleted(item.getProgressCompleted());r.setFailureReason(item.getFailureReason());r.setNextRetryAt(item.getNextRetryAt());r.setStartedAt(item.getStartedAt());r.setFinishedAt(item.getFinishedAt());return r; }
    private AiKnowledgeSearchHitResponse searchResponse(VectorSearchHit hit) { AiKnowledgeSearchHitResponse r=new AiKnowledgeSearchHitResponse();r.setSourceType(String.valueOf(hit.payload().get("sourceType")));r.setScore(hit.score());r.setTitle(String.valueOf(hit.payload().getOrDefault("title",hit.payload().getOrDefault("question",""))));r.setContent(String.valueOf(hit.payload().getOrDefault("content",hit.payload().getOrDefault("answer",""))));r.setLocation(String.valueOf(hit.payload().getOrDefault("location","")));r.setMetadata(hit.payload());return r; }
}
