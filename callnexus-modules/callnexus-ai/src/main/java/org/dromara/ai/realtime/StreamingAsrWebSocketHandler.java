...
public class StreamingAsrWebSocketHandler extends AbstractWebSocketHandler {
    private final StreamingAsrSessionManager sessionManager;

    @Override
    public void afterConnectionEstablished(WebSocketSession session) throws Exception {
        Map<String, String> query = UriComponentsBuilder.fromUri(session.getUri()).build()
            .getQueryParams().toSingleValueMap();
        try {
            sessionManager.connect(session, query.get("token"));
        } catch (Exception exception) {
            log.warn("拒绝流式 ASR 音频连接，remote={}，error={}", session.getRemoteAddress(), exception.getMessage());
            session.close(CloseStatus.POLICY_VIOLATION.withReason(
                StringUtils.blankToDefault(exception.getMessage(), "连接校验失败")));
        }
    }
